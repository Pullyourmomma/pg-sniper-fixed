...
def get_followers(random_number):
    ...
    data = response.json()
    times = 0
    while not "count" in data:
        if times <= 3:
            print(f"No count found for user ID {random_number}. Likely due to too many requests.")
            time.sleep(2.5)
            print("Retrying in 5...")
            time.sleep(1)
            print("4...")
            ...
            print("Didn't work within 3 retries. Setting followers to 9999999 to skip!")
            return 9999999
    count1 = data["count"]
    return count1
